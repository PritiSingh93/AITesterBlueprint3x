...
def get_mcp_server_params() -> StdioServerParameters:
    ...
    jira_url = os.getenv("JIRA_URL") or os.getenv("JIRA_BASE_URL", "https://bugzz.atlassian.net")
...
class GetIssueInput(BaseModel):
    issue_key: str = Field(description="Jira issue key, e.g. QATEST-7")


def wrap_get_issue(mcp_tool):
    """Expose only `issue_key` to the LLM.

    The MCP adapter marks every optional param of jira_get_issue as required,
    and Groq rejects a tool call that omits them (400 tool_use_failed).
    """

    class JiraGetIssue(BaseTool):
        name: str = "jira_get_issue"
        description: str = (
            "Fetch a Jira issue by its key (e.g. QATEST-7). Returns the summary, "
            "description, status, priority and comments."
        )
        args_schema: type[BaseModel] = GetIssueInput

        def _run(self, issue_key: str) -> str:
            return mcp_tool._run(issue_key=issue_key)
...
def run_crew(ticket_id: str):
    ...
    with MCPServerAdapter(server_params, connect_timeout=60) as mcp_tools:
        ...
        get_issue_tool = next(t for t in mcp_tools if t.name == "jira_get_issue")
        filtered_tools = [wrap_get_issue(get_issue_tool)]
...
if __name__ == "__main__":
    run_crew(os.getenv("BUG_ID", "QATEST-7"))
